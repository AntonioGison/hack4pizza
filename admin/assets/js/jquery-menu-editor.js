...
function MenuEditor(idSelector, options) {
    ...
    function TButtonGroup() {
        ...
        var $btnOut = TButton({classCss: 'btn btn-secondary btn-sm btnOut btnMove', text: '<i class="fa fa-level-down-alt clickable"></i>'});
        var $btnIn = TButton({classCss: 'btn btn-secondary btn-sm btnIn btnMove', text: '<i class="fa fa-level-up-alt clickable"></i>'});
    ...
    this.add = function(){
        ...
        var textItem = $('<input readonly style="border: 0px">').addClass('txt').val(data.text);
        var pageItem = $('<input hidden style="border: 0px">').addClass('href').val(data.href);
        var iconItem = $('<i>').addClass(data.icon);
        var div = $('<div>').css({"overflow": "auto"}).append(iconItem).append("&nbsp;").append(textItem).append(btnGroup).append(pageItem);
        var $li = $("<li>").data(data);
